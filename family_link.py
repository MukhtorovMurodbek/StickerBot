...
# partial deploy.
VERSION = os.environ.get("FAMILY_VERSION", "1.0.0")
...
def attach(app, bot_id: str, display_name: str, start_time: datetime) -> None:
    ...
    app.job_queue.run_repeating(_poll_commands, interval=COMMAND_POLL_SECONDS, first=COMMAND_POLL_SECONDS)
    ...
    logger.info("Family bus on: heartbeat every %ss, commands polled every %ss, tidy-up every %ss.",
                HEARTBEAT_SECONDS, COMMAND_POLL_SECONDS, HOUSEKEEPING_SECONDS)
